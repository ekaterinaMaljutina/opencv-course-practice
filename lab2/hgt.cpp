...
void drawKpt(Mat& img, const KeyPoint& p, const Scalar& color, int flags, Point offset = Point(0, 0))
{
  ...
  if (flags & DrawMatchesFlags::DRAW_RICH_KEYPOINTS)
  {
    ...
    circle(img, center, radius, color, 1, 16);
    ...
    if (p.angle != -1)
    {
      ...
      line(img, center, center + orient, color, 1, 16);
    }
  }
  else
  {
...
int main(int argc, char* argv[])
{
  ...
  ratioTestMatching(descriptorMatcher, descriptors1, descriptors2, matches, 0.6);
  ...
  //task 2
  vector<Point2f> __obj__, __scene__;
  //

  for (size_t matchInd = 0; matchInd < matches.size(); matchInd++)
  {
    //task 1
    KeyPoint __obj__keypoint = keypoints1[matches[matchInd].queryIdx];
    KeyPoint __scene__keypoint = keypoints2[matches[matchInd].trainIdx];
    float r = (float)norm(__obj__keypoint.pt - center);
    float image_angle = atan( (__obj__keypoint.pt.y - center.y) / (__obj__keypoint.pt.x - center.x));


    float delta_angel = (__scene__keypoint.angle - __obj__keypoint.angle) * CV_PI / 180.0f;
    float delta_scale = __scene__keypoint.size / __obj__keypoint.size;
    float scene_r = delta_scale * r;
    float scene_angle = delta_angel + image_angle;
    Point2f center_in_scene(
        __scene__keypoint.pt.x + scene_r * cos(scene_angle),
        __scene__keypoint.pt.y + scene_r * sin(scene_angle));
    hgt.at<int>( center_in_scene.y / splitSet , center_in_scene.x / splitSet ) += 1;
    //
    //task 2
    __obj__.push_back(keypoints1[matches[matchInd].queryIdx].pt);
    __scene__.push_back(keypoints2[matches[matchInd].trainIdx].pt);
    //
  }
  ...
  //task 2
  Mat H = findHomography(__obj__, __scene__, RANSAC);
  vector<Point2f> __obj__position;
  __obj__position.push_back(Point2f(0.f, 0.f));
  __obj__position.push_back(Point2f(image.cols, 0.f));
  __obj__position.push_back(Point2f(image.cols, image.rows));
  __obj__position.push_back(Point2f(0.f, image.rows));
  vector<Point2f> __position__in__scene(4);
  Scalar color(255,0,0);
  perspectiveTransform(__obj__position, __position__in__scene, H);
  line(scene, __position__in__scene[0],__position__in__scene[1],color, 4);
  line(scene, __position__in__scene[1], __position__in__scene[2],color, 4);
  line(scene,__position__in__scene[2],__position__in__scene[3],color, 4);
  line(scene, __position__in__scene[3],__position__in__scene[0],color, 4);
  //
  imshow("result", scene);
  waitKey(0);
